function [centers,contents,boxes,ind]=grid2D_Iceland(xydata, maxnumber, minnumber, minmagnitude, lim, minsize, saveflag, rootname);
%function [centers,contents,boxes,ind]=grid2D_Iceland(xydata, maxnumber, minnumber, minmagnitude, lim, minsize, saveflag, rootname);
%
...
for jj = 1:size(nb,2)
   ...
   if saveflag==1
      ...
      if b(jj)>=minnumber 
         filename=[rootname,'/',rootname '_' sprintf('%0.4d',jj)];
