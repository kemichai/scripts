function  [ind,bx,by] = qtree0(x,y,s,lim,n0,minsize)
...
ind = ones(size(x));
if or(N<=n0,box<2*minsize) % Altered from "box<=2*minsize" by JT, 10 Nov. 2010
  bx = 0.5; by = 0.5;
  return
end
...
[ind1,bx1,by1] = qtree0(x(n1),y(n1),s(n1),lm,n0,minsize);

n2 = find( x<x_mid & y>y_mid );     % Upper-left
lm = [lim(1)  x_mid  y_mid  lim(4)];
[ind2,bx2,by2] = qtree0(x(n2),y(n2),s(n2),lm,n0,minsize);

n3 = find( x>x_mid & y<=y_mid );    % Lower-right
lm = [x_mid  lim(2)  lim(3)  y_mid];
[ind3,bx3,by3] = qtree0(x(n3),y(n3),s(n3),lm,n0,minsize);

n4 = find( x>x_mid & y>y_mid );     % Upper-right
lm = [x_mid  lim(2)  y_mid  lim(4)];
[ind4,bx4,by4] = qtree0(x(n4),y(n4),s(n4),lm,n0,minsize);
